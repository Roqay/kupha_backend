...
class WebmasterSettingsController extends Controller
{
    ...
    public function update(Request $request)
    {
        ...
        if (!empty($WebmasterSetting)) {
            ...
            $OLD_BACKEND_PATH = config('roqay.backend_path');
            ...
            foreach ((array)$data as $key => $value) {
                ...
                foreach ($env as $env_key => $env_value) {
                    ...
                    if ($entry[0] == $key) {
                        ...
                        config(['roqay.' . strtolower($key) => $value]);
                    } else {
                        // If not, keep the old one
                        $env[$env_key] = $env_value;
                        config(['roqay.' . strtolower($env_key) => $env_value]);
                    }
                }
            }
    ...
    public function db_language_add($code)
    {
        ...
        try {
            ...
            if ($current_lang_code != "") {
                ...
                Menu::where('id', '>', 0)->update(['link_' . $code => DB::raw('link_' . $current_lang_code)]);
            }
        } catch (\Exception $e) {
        }
        try {
