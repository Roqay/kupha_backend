    ...
    public function list_page($lang, $WebmasterSection = [], $Category = [])
    {
        if (!empty($WebmasterSection)) {
            ...
            if ($WebmasterSection->type != 8 && $WebmasterSection->type != 7) {
                ...
                // order and paginate
                $TopicsList = $TopicsList->orderby('date', config('roqay.frontend_topics_order'))->orderby('id', config('roqay.frontend_topics_order'))->paginate(config('roqay.frontend_pagination'));
