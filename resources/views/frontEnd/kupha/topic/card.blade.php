@if (!empty($Topic))
    @php
        ...
        $image = !empty($Topic->photo_file)
            ? URL::to('uploads/topics/' . $Topic->photo_file)
            : asset('assets/images/news/banner.png');
    @endphp

    <div class="membership-item-block">
        <div class="banner-holder">
            <a href="{{ $topic_link_url }}">
                <img class="card-img-top" src="{{ $image }}" width="100%" height="100%" alt="{{ $title }}"
                    loading="lazy">
            </a>
        </div>
        <div class="content">
