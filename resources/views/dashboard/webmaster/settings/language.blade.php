        ...
        <div class="row">
            <div class="col-sm-6">
                ...
                <select name="date_format" class="form-control select2 select2-hidden-accessible" ui-jp="select2"
                        ui-options="{theme: 'bootstrap'}">
                    <option value="Y-m-d" {{ (config('roqay.date_format')=="Y-m-d")?"selected":"" }}>Y-m-d</option>
                    <option value="d-m-Y" {{ (config('roqay.date_format')=="d-m-Y")?"selected":"" }}>d-m-Y</option>
                    <option value="m-d-Y" {{ (config('roqay.date_format')=="m-d-Y")?"selected":"" }}>m-d-Y</option>
                    <option value="d/m/Y" {{ (config('roqay.date_format')=="d/m/Y")?"selected":"" }}>d/m/Y</option>
                    <option value="m/d/Y" {{ (config('roqay.date_format')=="m/d/Y")?"selected":"" }}>m/d/Y</option>
                    <option value="d.m.Y" {{ (config('roqay.date_format')=="d.m.Y")?"selected":"" }}>d.m.Y</option>
                    <option value="m.d.Y" {{ (config('roqay.date_format')=="m.d.Y")?"selected":"" }}>m.d.Y</option>
            ...
            <div class="col-sm-6">
                ...
                <select name="first_day_of_week" class="form-control select2 select2-hidden-accessible" ui-jp="select2"
                    ...
                    <option value="{{ $key }}" {{ (config('roqay.first_day_of_week')==$key)?"selected":"" }}>{{ $dayName }}</option>
                    @endforeach
                            ...
                            <td class="text-center">
                                @if(@Auth::user()->permissionsGroup->edit_status)
                                    ...
                                    <a class="btn btn-sm info " target="_blank"
                                       href="{{ url(config('roqay.backend_path').'/webmaster/translations/'.$Language->code.'/translations') }}">
